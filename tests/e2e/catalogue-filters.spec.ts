...
test.describe('Catalogue — sync URL ↔ filtres', () => {
  test('naviguer vers /catalogue?type=moto depuis la home met le filtre Moto actif', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('cliquer "Auto" DEPUIS le catalogue (déjà dessus) change les filtres sans reload', async ({
    page,
  }) => {
  ...
  test('cliquer "Moto" puis "Promos" depuis le catalogue enchaîne les filtres', async ({
    page,
  }) => {
    await page.goto('/catalogue');
