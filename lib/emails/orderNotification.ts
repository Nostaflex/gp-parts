...
/**
 * Email de notification interne au gérant (Stephane) à chaque commande.
 *
 * Distinct de buildOrderConfirmationEmail (orienté client « merci ») :
 * ici le ton est opérationnel « à traiter », et on expose les coordonnées
 * client (tel/email/adresse) pour que Stephane puisse rappeler / livrer.
 */
export function buildOrderNotificationEmail(order: Order): { subject: string; html: string } {
  const subject = `Nouvelle commande ${escapeHtml(order.orderNumber)} — ${escapeHtml(order.customer.firstName)} ${escapeHtml(order.customer.lastName)}`;
  ...
  const deliveryLabel =
    order.delivery.option === 'store-pickup'
      ? 'Retrait en boutique (Baie-Mahault)'
      : `Livraison à domicile — ${escapeHtml(order.delivery.address)}, ${escapeHtml(order.delivery.postalCode)} ${escapeHtml(order.delivery.city)}`;
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
          ...
          <tr>
            <td style="padding-bottom:24px;">
              <p style="margin:0;font-size:12px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#E87200;">
                CAR PERFORMANCE · ADMINISTRATION
              </p>
            </td>
          </tr>

          <tr>
            <td style="background:#1A0F06;border-radius:16px;padding:32px;text-align:center;">
              ...
              <h1 style="margin:0;font-size:24px;font-weight:900;color:#F4EDE0;line-height:1.3;">
          ...
          <tr>
            <td style="background:#fff;border-radius:12px;border:1px solid #E5DDD3;padding:20px 24px;">
              <p style="margin:0 0 12px;font-size:11px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#999;">
                Client
              </p>
              <p style="margin:0 0 4px;font-size:15px;font-weight:700;color:#1A0F06;">
                ${escapeHtml(order.customer.firstName)} ${escapeHtml(order.customer.lastName)}
              </p>
              <p style="margin:0 0 4px;font-size:14px;color:#1A0F06;">
                Tél : <a href="tel:${escapeHtml(order.customer.phone)}" style="color:#E87200;text-decoration:none;">${escapeHtml(order.customer.phone)}</a>
              </p>
              <p style="margin:0;font-size:14px;color:#1A0F06;">
                Email : <a href="mailto:${escapeHtml(order.customer.email)}" style="color:#E87200;text-decoration:none;">${escapeHtml(order.customer.email)}</a>
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="text-align:center;">
              <p style="margin:0;font-size:12px;color:#999;">
                Commande enregistrée dans le back-office · statut « Nouvelle »
              </p>
            </td>
          </tr>
